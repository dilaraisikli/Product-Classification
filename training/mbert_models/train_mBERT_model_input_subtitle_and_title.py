...
EPOCHS = 7  
LEARNING_RATE = 1e-5
SAVE_PATH = "./mbert_product_classifier_only_title"
DATA_PATH = "./dataset_splits"

# ------------------- DATA LOADING & SPLITTING -------------------
df = pd.read_csv('/home/ubuntu/product_classifier/di-interview-product-classifier/products/cleaned_data.csv')


df["title_clean"] = df["title_clean"].fillna("").astype(str)
df["subtitle_clean"] = df["subtitle_clean"].fillna("").astype(str)

df["text"] = df.apply(lambda row: row["title_clean"] if row["subtitle_clean"] == "" 
...
labels = df["productType"].unique()
label2id = {label: idx for idx, label in enumerate(labels)}
id2label = {idx: label for label, idx in label2id.items()}

df["label"] = df["productType"].map(label2id).astype(int)  
...
print(f" Train, Val, and Test datasets saved in {DATA_PATH}")
...
# ------------------- TOKENIZATION ------------------- (Text Preprocessing for BERT)
tokenizer = BertTokenizer.from_pretrained(MODEL_NAME) # Loads the BERT tokenizer 
...
def evaluate():
    global id2label  # Ensure `id2label` is used from the global scope
    ...
    # Convert `id2label` values to strings
    ...
    print(classification_report(all_labels, all_preds, target_names=[id2label[i] for i in unique_labels]))
